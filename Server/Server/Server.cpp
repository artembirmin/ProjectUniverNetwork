...
int getFileSize() {
	ifstream file1("C:\\Users\\petra\\source\\repos\\ProjectNetwork\\Client\\Client\\File1.txt", ios::binary | ios::in);//октрываем файл для чтения 
...
void ReadingStudentData(int fileSize) {
	...
	file2.read((char*)&studentsList, sizeof(studentsList));//считали данные о студенте,записанные клиентом

}

void writeStudentGrade() {
	ofstream file("File2.txt", ios::binary | ios::app);//Открытие файла для двоичных операций ввода-вывода | Добавление всех выводимых данных в конец заданного файла
...
int main()
{
	while (true) {
		...
		ReadingStudentData(currentFileSize);
		writeStudentGrade();
	}
}
